...
export const onCreateStartedCheckoutClientEvent = (email: string, path: string) => {
    ...
    const data: IKlaviyoEventBodyParams = {
      ...
      attributes: {
        ...
        properties: {
          ...
          CheckoutURL: path,
          Items: state.cart.cartData?.items.map(item => ({
            ProductID: item.id,
            SKU: item.sku,
...
export const onSendSlackMessage = (channel: string, username: string, icon_emoji: string, text: string, buttons?: {text: string, url: string}[]) => {
  return async () => {
