...
const images = {
  estante: {
    ...
    media: 'https://imagedelivery.net/7yveHullsFjmXtPLdJPFsg/da8b540e-6411-468f-a73d-4a4534152100/fit=cover',
  ...
  cabinet: {
    ...
    media: 'https://imagedelivery.net/7yveHullsFjmXtPLdJPFsg/f745f0a2-5b8f-42b0-4ef3-9337b9ced700/fit=cover',
  ...
  CabinetAbierta: {
    ...
    media: 'https://imagedelivery.net/7yveHullsFjmXtPLdJPFsg/f0ed1be8-3200-48e0-0436-714d2055ad00/fit=cover',
...
const ShelfBuilder: React.FC<ShelfBuilderProps> = ({
  ...
  useEffect(() => {
    if (!canvasRef.current || !imagesLoaded || Object.keys(imageCache.current).length === 0) return;
    ...
    const drawImages = () => {
      ...
      const usedColumns = Math.max(...shelfConfigurations.map(config => config.position.column));
      const usedRows = Math.max(...shelfConfigurations.map(config => config.position.row));
      ...
      const horizontalOffset = Math.max(0, (canvasSize.width - totalContentWidth) / 2);
      const verticalOffset = Math.max(0, (canvasSize.height - totalContentHeight) / 2);
      ...
      const getImageUrl = (shelf: any) => {
        switch(shelf.children.attributes[propsNames.configuracion]) {
          case 'estante':
            return images.estante[shelf.children.attributes[propsNames.alto] as 'alta' | 'media' | 'baja'];
          case 'cabinet':
            return images.cabinet[shelf.children.attributes[propsNames.alto] as 'alta' | 'media' | 'baja'];
          case 'nicho':
            return images.CabinetAbierta[shelf.children.attributes[propsNames.alto] as 'alta' | 'media' | 'baja'];
          default:
      ...
      shelfConfigurations.forEach((shelf) => {
        ...
        if (cachedImage && cachedImage.width > 0 && cachedImage.height > 0) {
          ...
          if (!columnMaxHeights[column] || drawHeight > columnMaxHeights[column]) {
      ...
      const adjustedHorizontalOffset = (canvasSize.width - (maxColumnWidth * usedColumns)) / 2;
      ...
      recalculatedPositions.forEach((shelf) => {
        ...
        const x = adjustedHorizontalOffset + 
        ...
        if (cachedImage && cachedImage.width > 0 && cachedImage.height > 0) {
          ...
          if (isCard && (shelf.moduleId !== editingModuleId) && !isPreConfigView) {
          ...
          //Tooltip para editar
          if (!isCart && !isCard && shelf.moduleId === editingModuleId && shelfConfigurations.length > 1 && !isPreConfigView) {
            ...
            const moduleIndex = shelfConfigurations.findIndex(m => m.moduleId === shelf.moduleId);
            const moduleNumber = moduleIndex + 1;
            const moduleText = moduleNumber === 1 ? 'Primer módulo' : 
                             moduleNumber === 2 ? 'Segundo módulo' :
                             moduleNumber === 3 ? 'Tercer módulo' :
                             moduleNumber === 4 ? 'Cuarto módulo' :
                             moduleNumber === 5 ? 'Quinto módulo' :
                             `${moduleNumber}° módulo`;
            ...
            tempCtx.quadraticCurveTo(bubbleX + bubbleWidth, bubbleY, bubbleX + bubbleWidth, bubbleY + 8);
            tempCtx.lineTo(bubbleX + bubbleWidth, bubbleY + bubbleHeight - 8);
            tempCtx.quadraticCurveTo(bubbleX + bubbleWidth, bubbleY + bubbleHeight, bubbleX + bubbleWidth - 8, bubbleY + bubbleHeight);
            tempCtx.lineTo(bubbleX + 8, bubbleY + bubbleHeight);
            tempCtx.quadraticCurveTo(bubbleX, bubbleY + bubbleHeight, bubbleX, bubbleY + bubbleHeight - 8);
            ...
            tempCtx.fillText(moduleText, bubbleX + bubbleWidth/2, bubbleY + bubbleHeight/2);

            // Dibujar la flecha apuntando hacia abajo
            tempCtx.beginPath();
            tempCtx.moveTo(bubbleX + bubbleWidth/2, bubbleY + bubbleHeight + 10);
            tempCtx.lineTo(bubbleX + bubbleWidth/2 - 10, bubbleY + bubbleHeight);
            tempCtx.lineTo(bubbleX + bubbleWidth/2 + 10, bubbleY + bubbleHeight);
            tempCtx.closePath();
            tempCtx.fillStyle = '#000000';
            tempCtx.fill();
  ...
  }, [shelfConfigurations, images, maxRows, maxColumns, canvasSize, imagesLoaded, isCart, editingModuleId, isCard]);
