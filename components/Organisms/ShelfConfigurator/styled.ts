...
export const ModuleContainer = styled.div<{ $isOpen?: boolean; $hasError?: boolean }>`
  position: relative;
  z-index: ${props => props.$hasError ? 10 : 1};
  border: 1px solid ${props => props.$hasError ? props.theme.colors.rareRed : props.$isOpen ? props.theme.colors.whiteEdgar : props.theme.colors.whiteEdgar};
  padding: 16px;
  border-radius: 8px;
...
/*     transform: scale(1.002); */
    ...
    ${Checkbox} {
      border-color: ${props => props.$hasError ? props.theme.colors.rareRed : props.$isOpen ? props.theme.colors.yellowCalm : props.theme.colors.paternoster};
...
export const IconContainer = styled.div<{ $isHovered?: boolean }>`
  ...
  border: 2px solid ${props => props.$isHovered ? props.theme.colors.black : props.theme.colors.white};
  width: 42px;
  height: 42px;
  cursor: pointer;
...
export const ConfigImage = styled.div<{ $isSelected: boolean; $isDisable?: boolean }>`
  display: flex;
  justify-content: center;
  align-items: center;
  border: 2px solid ${props => props.$isSelected ? props.theme.colors.yellowCalm : props.theme.colors.paternoster};
  border-radius: 8px;
  padding: 1px 0px;
...
export const TitleAndDelete = styled.div<{ $isPreconfig?: boolean, $isOpen?: boolean }>`
  display: flex;
  ${props => props.$isOpen && `
     cursor: pointer;
  `}
  
  ${props => props.$isPreconfig && `
    flex-direction: row;
    gap: 10px;
  `}
  ${props => !props.$isPreconfig && `
...
export const ListItem = styled.li<{ $isLast: boolean; $isFirst: boolean; $isSelected?: boolean; $isDisable?: boolean }>`
  padding: 8px 16px;
  cursor: ${props => props.$isDisable ? 'not-allowed' : 'pointer'};
  background-color: ${props => props.$isDisable ? props.theme.colors.millionGray : 'transparent'};
  opacity: ${props => props.$isDisable ? '0.5' : '1'};

  &:hover {
    background-color: ${props => props.$isDisable ? props.theme.colors.millionGray : props.theme.colors.coldMorning};
  }
`;
